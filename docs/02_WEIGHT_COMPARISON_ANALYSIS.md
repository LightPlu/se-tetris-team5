...
|------|-----------------|-----------------|------|--------|
| **Landing Height** | -4.50 | -8.68 | **1.93배** | 더 큰 패널티 (높이에 더 민감) |
| **EPCM** | 3.42 | 4.36 | **1.27배** | 약간 큰 보너스 |
| **Row Transitions** | -3.22 | -2.21 | **0.69배** | 더 작은 패널티 (가로 전환에 덜 민감) |
| **Column Transitions** | -9.35 | -5.30 | **0.57배** | 더 작은 패널티 (세로 전환에 덜 민감) |
| **Holes** | -7.90 | -1.34 | **0.17배** | **훨씬 작은 패널티** (구멍에 덜 민감) |
| **Well Sums** | -3.39 | -10.04 | **2.96배** | 더 큰 패널티 (Well에 더 민감) |

### 대전 모드 공격 보너스 (현재 구현만 존재)

| 특징 | 가중치 | 설명 |
|------|--------|------|
| **Attack 2 Lines** | 32.90 | 2줄 삭제 시 공격 보너스 |
| **Attack 3 Lines** | 20.52 | 3줄 삭제 시 공격 보너스 |
| **Attack 4 Lines (Tetris)** | 70.52 | 4줄 삭제 시 공격 보너스 (가장 큰 보너스) |
...
**차이점 분석**:
- 현재 구현이 **훨씬 작은 패널티** (-1.34 vs -7.90, 약 0.17배)
- **이유**: 대전 모드에서는 공격이 생존보다 중요
- 구멍이 있어도 공격을 보내면 상대방이 먼저 게임 오버될 수 있음
- 따라서 구멍에 덜 민감하게 반응
...
**차이점 분석**:
- 3줄 삭제 보너스가 2줄보다 작음 (20.52 vs 32.90)
...
**현재 구현:**
- Attack 4 Lines (Tetris) = 70.52 (양수, 매우 큰 값)
- 4줄 삭제 시 큰 보너스
...
**이유:**
- 대전 모드에서는 **공격이 생존보다 중요**
- 구멍이 있어도 공격을 보내면 상대방이 먼저 게임 오버될 수 있음
- 따라서 구멍에 대한 패널티를 줄이고 공격에 집중
...
**현재 구현:**
- Row Transitions: -2.21 (약 0.69배)
- Column Transitions: -5.30 (약 0.57배)
...
### 현재 구현 평가 함수 (최신 가중치)
```java
Score = -8.68 × Landing Height
      + 4.36 × EPCM (Eroded Piece Cells Metric)
      - 2.21 × Row Transitions
      - 5.30 × Column Transitions
      - 1.34 × Number of Holes
      - 10.04 × Well Sums
      + (linesCleared == 2 ? 32.90 : 0)
      + (linesCleared == 3 ? 20.52 : 0)
      + (linesCleared == 4 ? 70.52 : 0)  // Tetris 보너스
```
...
**가중치 특징**:
- Holes: 작은 패널티 (-1.34) → 구멍 허용
- Well Sums: 큰 패널티 (-10.04) → Well 회피
- Attack 보너스: 매우 큰 보너스 (70.52 for Tetris) → 공격 장려
